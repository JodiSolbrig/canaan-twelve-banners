# Twelve Banners — Tribes & Leader Progressions

## Tribe Overview

| Tribe     | Faith | Warriors | Goods | Loyalty | Playstyle                                                                 | Unique Action |
|-----------|-------|----------|-------|---------|---------------------------------------------------------------------------|---------------|
| Judah     | 3     | 3        | 3     | 3       | Natural group leader. Excels at becoming Champion and converting group success into personal Glory. | **Rally the Tribes** – Spend 1 Faith → Give +1 temporary Influence to one other player this round. |
| Benjamin  | 2     | 5        | 2     | 3       | Aggressive combat specialist. High risk, high reward on Military Track.   | **Raid** – Spend 1 Warrior → Gain 1 Goods + 1 Glory. If Military Track is in Low zone, lose 1 Loyalty instead of gaining Glory. |
| Levi      | 5     | 2        | 2     | 4       | Covenant guardian. Strongest at Moral/Spiritual Track and protecting the shared meter. | **Intercede** – Spend 1 Faith → Raise Covenant Meter by 1 or protect it from the next drop this round. |
| Ephraim   | 3     | 2        | 4     | 3       | Economic engine and political influencer (Deborah/Abdon tradition).       | **Double Portion** – Spend 1 Goods → Gain 2 Goods or 1 Goods + 1 other resource. |
| Manasseh  | 3     | 3        | 3     | 5       | Tanky stabilizer. Hard to knock down; strong when tracks fail.            | **Hold the Line** – Spend 1 Warrior or 1 Faith → Reduce the Covenant penalty of one failed track by 1 this round. |
| Reuben    | 2     | 4        | 3     | 3       | Early momentum and scouting (pastoral warrior tradition, Gilead ties).    | **Scout Ahead** – Exhaust 1 Warrior → Look at the top card of the Crisis modifier deck. |
| Simeon    | 2     | 4        | 3     | 4       | High-risk skirmisher. Southern aggressive tradition.                      | **Skirmish** – Spend 1 Warrior → Gain 1 Glory. If Military is Low, also gain 1 Goods. |
| Dan       | 3     | 4        | 2     | 3       | Trickster / strongman (Samson tradition). Flexible and disruptive.        | **Serpent’s Wisdom** – Spend 1 Faith → Discard and redraw one pending Crisis modifier card (once per game). |
| Naphtali  | 3     | 3        | 3     | 3       | Flexible support (Barak tradition). Good at repositioning and helping others. | **Reposition** – Move 1 of your already-placed Influence tokens to a different track (before resolution). |
| Gad       | 2     | 4        | 3     | 4       | Frontline tank (Gilead warrior tradition).                                | **Stand Firm** – Spend 1 Warrior → Protect yourself from the next Loyalty loss this round. |
| Asher     | 2     | 2        | 5     | 3       | Economic powerhouse (coastal abundance tradition).                        | **Harvest** – Spend 1 Faith or rest → Gain 2 Goods. |
| Issachar  | 4     | 2        | 3     | 3       | Strategic planner (Tola’s stabilizing role). Information and timing advantage. | **Study the Times** – Spend 1 Faith → Look at the top two Crisis modifier cards and put them back in any order. |
| Zebulun   | 2     | 3        | 4     | 2       | Merchant engine (coastal/trade tradition, Elon).                          | **Bargain** – Spend 1 Goods → Perform two standard resource conversions this turn. |

## Leader Progressions (3 upgrades per tribe)

### Judah — Othniel
1. **Othniel I – Lion’s Rally**: When you become Champion of any track, gain +1 extra Glory.  
2. **Othniel II – Wholehearted Charge**: Once per round, when investing in Military, your tokens count as +1.  
3. **Othniel III – First in Line**: Once per game, after Influence is revealed, you may move 1 of your tokens to a different track.

### Benjamin — Ehud
1. **Ehud I – Left-Handed Strike**: When you become Military Champion, gain +1 Warrior.  
2. **Ehud II – Hidden Dagger**: Once per round, you may treat 1 Warrior as 2 Influence on the Military Track.  
3. **Ehud III – Deliverer’s Legacy**: After winning Military Champion, you may immediately take a free Recruit action.

### Levi — Phinehas
1. **Phinehas I – Covenant Zeal**: When you become Moral Champion, raise Covenant Meter by 1.  
2. **Phinehas II – Intercession**: Once per round, spend 1 Faith to prevent 1 Covenant loss.  
3. **Phinehas III – Sacred Inquiry**: Once per game, after all Influence is placed, look at the next Crisis modifier card.

### Ephraim — Deborah / Abdon
1. **Deborah – Prophetic Voice**: Once per round, when investing in Moral Track, gain +1 Faith if you become Champion.  
2. **Abdon I – Many Sons**: When you unlock this, gain 1 Goods permanently to your starting total for the rest of the game.  
3. **Abdon II – Landed Authority**: Once per round, you may convert 2 Goods into 1 Faith or 1 Warrior.

### Manasseh — Gideon / Jair / Jephthah
1. **Gideon – Fleece Test**: Once per round, before placing Influence, look at one track’s current special modifier (if any).  
2. **Jair – Thirty Towns**: Gain +1 Goods when the Provision Track reaches High.  
3. **Jephthah – Outcast’s Resolve**: When a track you invested in fails, you lose 1 less Loyalty (minimum 0).

### Reuben
1. **Reubenite Scout – Firstborn Advance**: Once per round, you may place 1 token after seeing one other player’s placement.  
2. **Gilead Tie I – Pathfinder**: When you invest at least 2 tokens on any track, gain 1 temporary Influence on a second track.  
3. **Gilead Tie II – Bold Claim**: Once per game, after resolution, gain +1 Glory if you were not Champion but invested the second-most.

### Simeon
1. **Simeonite Raider – Vengeful Strike**: When you become Military Champion, gain +1 Warrior.  
2. **Furious Assault**: After a Military Track failure you invested in, you may immediately place 1 token on it next round for free.  
3. **Raid Leader**: Once per round, convert 1 Goods into 1 Warrior or vice versa at no loss.

### Dan — Samson
1. **Samson I – Nazirite Strength**: When you invest 3 tokens on Military, treat them as 4.  
2. **Samson II – Riddle & Cunning**: Once per round, you may move 1 of your tokens after all others are revealed.  
3. **Samson III – Final Stand**: Once per game, when a track fails, you may spend 2 Warriors to ignore the Covenant loss for everyone.

### Naphtali — Barak
1. **Barak I – Doe’s Leap**: Once per round, after Influence is placed, you may move 1 of your tokens to another track.  
2. **Barak II – Swift Response**: When you become Champion, you may give 1 temporary Influence to another player next round.  
3. **Barak III – Northern Alliance**: Once per game, two of your tokens on different tracks both count as +1.

### Gad
1. **Gadite Warrior – Raider’s Resolve**: Reduce all Loyalty loss you suffer by 1 (minimum 0).  
2. **Enduring Defense**: When Military Track is Low, your tokens on it count as +1.  
3. **Overcomer**: Once per game, ignore one setback (failed track penalty or Selfish Glory failure).

### Asher
1. **Blessed Abundance**: When Provision Track reaches High, gain +1 extra Goods.  
2. **Fertile Blessing**: Once per round, when you take a Generate Goods action, gain 1 additional Goods.  
3. **Rich Harvest**: Once per game, double the Goods you gain from any single action or Champion reward.

### Issachar
1. **Understanding of Times**: Once per round, before placing tokens, look at one track’s current threshold status.  
2. **Strategic Insight**: When you become Champion of any track, gain +1 Faith.  
3. **Wise Counsel**: Once per game, after all tokens are placed, you may force one other player to move 1 of their tokens.

### Zebulun
1. **Sea Trader**: Once per round, convert 1 Faith ↔ 1 Goods at 1:1.  
2. **Commerce Route**: When you become Provision Champion, gain +1 Glory.  
3. **Profitable Venture**: Once per game, after gaining Goods from any source, gain that many again.

---

**Design Notes**  
Source: Book of Judges as primary authority. All named leaders are biblically attested.  
Progression: Exactly 3 upgrades per tribe (flexible unlock structure allowed).  
Resources: Faith (special abilities / Covenant), Warriors (combat / defense), Goods (growth / economy).  
Loyalty starts varied to reflect tribal resilience themes.
